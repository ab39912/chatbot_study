...
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:transparent;color:#1e293b;font-size:14px;overflow:hidden;}

/* ── tap overlay ─────────────────────────────────────────────── */
#tap{position:fixed;inset:0;display:flex;flex-direction:column;align-items:center;
  justify-content:center;gap:14px;cursor:pointer;background:transparent;z-index:10;}
#tap .ring{width:80px;height:80px;border-radius:50%;border:2px solid rgba(14,165,233,.35);
...
#tap .label{font-size:13px;font-weight:600;color:#64748b;letter-spacing:.4px;}
#tap .hint{font-size:11px;color:#94a3b8;}
...
#toast{display:none;position:fixed;bottom:16px;left:50%;transform:translateX(-50%);
  background:#fef2f2;border:1px solid rgba(220,38,38,.35);border-radius:10px;
  padding:10px 18px;font-size:12px;color:#b91c1c;text-align:center;
  max-width:360px;line-height:1.5;z-index:20;}

/* ── session screen ──────────────────────────────────────────── */
#sess{display:none;padding:16px 16px 12px;}
.sess-hd{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;}
...
.rec-ind{display:none;align-items:center;gap:5px;font-size:11px;font-weight:700;color:#dc2626;}
.rec-dot{width:8px;height:8px;border-radius:50%;background:#dc2626;animation:blink 1.5s ease-in-out infinite;display:inline-block;}
@keyframes blink{0%,100%{opacity:1;}50%{opacity:.2;}}
.grid{display:grid;grid-template-columns:1fr 60px 1fr;gap:10px;align-items:center;margin-bottom:10px;}
.panel{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:18px 10px;
  display:flex;flex-direction:column;align-items:center;gap:7px;
  min-height:162px;justify-content:center;transition:background .25s,border-color .25s;}
...
.p-ico{width:48px;height:48px;border-radius:50%;background:#f1f5f9;display:flex;
  align-items:center;justify-content:center;font-size:26px;color:#94a3b8;
...
.status-p{text-align:center;font-size:12px;color:#94a3b8;min-height:15px;margin-bottom:6px;}
.irow{min-height:26px;display:flex;align-items:center;justify-content:center;margin-bottom:10px;}
.ipill{display:none;background:#f8fafc;border:1px solid #e2e8f0;border-radius:50px;
  padding:3px 11px;font-size:11px;color:#64748b;font-style:italic;
  max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.btn-end{background:transparent;color:#64748b;border:1px solid #e2e8f0;padding:8px 24px;
  border-radius:50px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;}
.btn-end:hover{background:#f1f5f9;}

/* ── end screen ──────────────────────────────────────────────── */
#endscr{display:none;padding:32px 16px;flex-direction:column;align-items:center;gap:10px;}
.end-logo{width:58px;height:58px;border-radius:50%;background:#f0fdf4;
  border:1px solid rgba(34,197,94,.3);display:flex;align-items:center;
  justify-content:center;font-size:28px;color:#16a34a;}
.end-h{font-size:20px;font-weight:600;text-align:center;}
.dl-row{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin:4px 0;}
.btn-dl{background:#f8fafc;color:#475569;border:1px solid #e2e8f0;padding:8px 18px;
  border-radius:50px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;}
.btn-dl:hover:not(:disabled){background:#f1f5f9;}
.btn-dl:disabled{opacity:.45;cursor:not-allowed;}
.btn-new{background:#f0f9ff;color:#0369a1;border:1px solid rgba(14,165,233,.4);
  padding:9px 28px;border-radius:50px;font-size:13px;font-weight:600;
  cursor:pointer;font-family:inherit;}
.btn-new:hover{background:#e0f2fe;}
</style>
</head>
<body>

<!-- TAP OVERLAY -->
<div id="tap" onclick="beginSession()">
  <div class="ring">🎙️</div>
  <div class="label">Tap anywhere to begin</div>
  <div class="hint">Section I — First Encounter</div>
</div>
...
<div id="sess">
  <div class="sess-hd">
    ...
    <div style="display:flex;align-items:center;gap:10px;">
      <span id="rec-ind" class="rec-ind"><span class="rec-dot"></span>REC</span>
...
<!-- END SCREEN -->
<div id="endscr" style="display:none;flex-direction:column;align-items:center;padding:32px 16px;gap:10px;">
  <div class="end-logo">✓</div>
  <div class="end-h">Session Complete</div>
  <p style="font-size:13px;color:#64748b;text-align:center;">Download before starting a new session.</p>
  <div class="dl-row">
    <button class="btn-dl" onclick="downloadTranscript()">📄 Transcript (.txt)</button>
    <button class="btn-dl" id="btn-audio" onclick="downloadAudio()" disabled>🎙️ Recording (.webm)</button>
  </div>
  <p id="rec-note" style="font-size:11px;color:#94a3b8;min-height:16px;text-align:center;"></p>
...
function initSTT(){
  ...
  recog.onstart=()=>{recogOn=true;phase='listening';setPanel('pa',false);setWave('pa');setStatus('Listening for participant…');$('pa-st').textContent='Listening…';};
  recog.onresult=evt=>{
    let interim='';
    ...
    setInterim(interim||''); if(interim)setPanel('pa',true);
  };
  recog.onend=()=>{recogOn=false;if(phase==='listening'&&!curTx.trim())setTimeout(()=>{if(phase==='listening')safeStart();},600);};
  recog.onerror=e=>{
    recogOn=false;
    if(e.error==='not-allowed'){setStatus('Mic denied — allow microphone access.');return;}
...
async function processTurn(text){
  ...
  try{
    const res=await fetch(API_URL,{method:'POST',headers:{'Content-Type':'application/json','x-api-key':API_KEY,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:180,system:SYS,messages:hist})});
    ...
    }else{if(phase!=='ended')listenForParticipant();}
  }catch(err){
...
async function startRec(){
  try{
    ...
    const n=$('rec-note');if(n)n.textContent='Audio recording was unavailable.';
  }
}
...
function beginSession(){
  // Hide overlay, show session screen — this tap IS the user gesture
  $('tap').style.display='none';
  $('sess').style.display='block';

  if(!window.speechSynthesis){
    showToast('Speech synthesis not available. Please use Chrome or Edge.');
    return;
  }
  if(!initSTT()){
    showToast('Speech recognition not available. Please use Chrome or Edge (not the VS Code built-in browser).');
  }

  sessStart=Date.now(); txLog=[]; recChunks=[];
  logTx('AI',OPENING);
  hist=[{role:'assistant',content:OPENING}];
  $('panel-ai').className='panel'; $('panel-pa').className='panel';
  $('ai-st').textContent='Ready'; $('pa-st').textContent='Waiting';
  setWave(null); setStatus('Starting…'); setInterim('');
  startRec();
  startTimer();
...
function endSession(){
  phase='ended'; stopSTT(); stopRec();
  clearInterval(tid); clearInterval(bugT);
  window.speechSynthesis.cancel();
